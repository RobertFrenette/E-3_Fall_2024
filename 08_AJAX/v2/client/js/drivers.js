// Driver functions
async function getDriversForTeam(team) {
  let drivers = team.getDrivers();
  if (drivers.length == 0) {
    console.log("Calling API to fecth Drivers");

    // Fetch Drivers from external API (AJAX)
    const driversResp = await fetch(
      `http://localhost:3000/api/driver/team/${team.getId()}`
    );
    if (!driversResp.ok) throw new Error();
    const driversJson = await driversResp.json();
...
async function populateDriverSelect(driverSelect, team) {
  ...
  const drivers = await getDriversForTeam(team);
